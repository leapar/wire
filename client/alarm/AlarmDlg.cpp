...
BOOL CAlarmDlg::OnInitDialog()
{
	...
	m_Password.Format("a123456A");
	m_UserName.Format("admin");
	m_ctrlDeviceIP.SetAddress(172,29,225,133);
    UpdateData(FALSE);
    g_pMainDlg = this;
...
void CAlarmDlg::OnConnectDevice() 
{
	...
	strcpy(struLoginInfo.sDeviceAddress,szDeviceIP); //设备地址，IP 或者普通域名 
//byRes1 保留，设为 0 
struLoginInfo.wPort = (WORD)m_DevicePort; //设备端口号，例如：8000 
strcpy(struLoginInfo.sUserName,szUserName);//登录用户名，例如：admin 
strcpy(struLoginInfo.sPassword,szPassword);//登录密码，例如：12345 
struLoginInfo.cbLoginResult = NULL;//登录状态回调函数，bUseAsynLogin 为1时有效 
struLoginInfo.pUser = NULL;//用户数据 
struLoginInfo.bUseAsynLogin = 0;//是否异步登录：0- 否，1- 是 
//byRes2 //保留，置为0 
	/*
	lUserId = NET_DVR_Login_V40(&struLoginInfo,&struLocalDeviceInfo);
	if(lUserId < 0)
	{
...
byIPv6Mode 
IPv6分配方式：0-路由公告，1-手动设置，2-启用DHCP分配 
struDnsServer1IpAddr 
域名服务器1的IP地址 
struDnsServer2IpAddr 
域名服务器2的IP地址 
byIpResolver 
IP解析服务器域名或IP地址（8000设备不支持域名） 
wIpResolverPort 
IP解析服务器端口号 
wHttpPortNo 
HTTP端口号 
...
Onvif多播搜索(SADP)：0- 默认，1- 启用，2- 禁用 
byRes 
保留，置为0
	if (NET_DVR_SetDVRConfig(g_struDeviceInfo[m_iDeviceIndex].lLoginID, NET_DVR_SET_NETCFG_V30, 0xFFFFFFFF, &strtNetcfg, sizeof(NET_DVR_NETCFG_V30)))
	{
		g_pMainDlg->AddLog(m_iDeviceIndex, OPERATION_SUCC_T, "NET_DVR_SET_TIMECFG");	
	}
	else
	...
	DWORD lpBytesReturned = 0;
	  bResult = NET_DVR_GetDVRConfig(g_struDeviceInfo[m_iDeviceIndex].lLoginID,
		  NET_DVR_GET_NETCFG_V30,
        0, &strtNetcfg,       sizeof(strtNetcfg),
  &lpBytesReturned);
	 err = NET_DVR_GetLastError();*/
	...
	//long ret = NET_DVR_StartListen_V30( ip,7200);
	//err = NET_DVR_GetLastError();

	long ret =   NET_DVR_StartListen_V30( ip,7200,callBack,NULL);
